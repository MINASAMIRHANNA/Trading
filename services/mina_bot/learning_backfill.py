...
Purpose:
- Backfill learning_stats_daily + threshold_adjustments_daily for the last N days.

This fixes the earlier error:
  sqlite3.OperationalError: no such column: day_utc
by using the correct column name used in this project: learning_stats_daily.date

Usage:
  python learning_backfill.py --db /path/to/bot_data.db --days 14

Optional:
  --clear   (delete existing learning_stats_daily rows for the target days before backfill)
...
def _delete_learning_day(conn, day: str) -> None:
    cols = _table_cols(conn, "learning_stats_daily")
    col = "date" if "date" in cols else ("day_utc" if "day_utc" in cols else None)
    if not col:
        return
    conn.execute(f"DELETE FROM learning_stats_daily WHERE {col}=?", (day,))
...
def _ensure_zero_row(db: DatabaseManager, day: str, strategy: str, exit_profile: str) -> None:
    ...
    with db.lock:
        db.conn.execute(
            """
            INSERT OR IGNORE INTO learning_stats_daily
...
def main():
    ...
    ap.add_argument("--clear", action="store_true", help="Clear existing learning_stats_daily for those days first")
    args = ap.parse_args()
