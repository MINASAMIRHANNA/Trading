...
def _ensure_learning_tables(db: Any) -> None:
    # Preferred path: DatabaseManager already owns the canonical schema DDL.
    try:
        fn = getattr(db, "ensure_schema", None)
        if callable(fn):
            fn()
            return
    except Exception:
    ...
    try:
        with db.lock:
            ...
            db.cursor.execute(
                "CREATE UNIQUE INDEX IF NOT EXISTS idx_learning_stats_key ON learning_stats_daily(day_utc, strategy, exit_profile)"
            )
            ...
            db.cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_threshold_adjust_day ON threshold_adjustments_daily(day_utc)"
            )
            db.conn.commit()
...
def store_learning_stats(db: Any, stats: Dict[Tuple[str, str], Dict[str, Any]], day_utc: Optional[str] = None) -> None:
    """
    Persist learning stats. Prefers sqlite table `learning_stats_daily`, otherwise falls back to a settings blob.
    """
    day = day_utc or _utc_day_str()
        ...
        try:
            ...
            with db.lock:
                for (strategy, exit_profile), s in stats.items():
                    ...
                    db.cursor.execute(
                        """
                        INSERT OR REPLACE INTO learning_stats_daily
            ...
            _log(db, f"learning_engine: failed to persist learning_stats_daily table: {e}", "WARNING")

    # Fallback: settings blob
    blob: Dict[str, Any] = {}
    for (strategy, exit_profile), s in stats.items():
        blob[f"{strategy}::{exit_profile}"] = s
    _set_setting(db, f"learning_stats_daily:{day}", blob)
...
def adapt_thresholds(db: Any, day_utc: Optional[str] = None) -> Dict[Tuple[str, str], int]:
    ...
    try:
        ...
        if _db_has_sqlite_handles(db):
            with db.lock:
                for (strategy, exit_profile), adj in adjustments.items():
                    ...
                    db.cursor.execute(
                        """
                        INSERT INTO threshold_adjustments_daily
        ...
        _log(db, f"learning_engine: failed to write threshold_adjustments_daily table: {e}", "WARNING")

    # Always keep a "current" blob for fast reads
    blob = {f"{k[0]}::{k[1]}": v for k, v in adjustments.items()}
    _set_setting(db, "threshold_adjustments_daily", blob)
