...
and cross-check with the bot DB (sqlite legacy OR Postgres PRIMARY).
...
def detect_trade_table(cur) -> str:
    for t in ("trades", "trade_state", "positions", "paper_trades"):
        if table_exists(cur, t):
            return t
    raise RuntimeError("Could not detect a trades table (expected trades/trade_state/positions).")


def fetch_trade_from_db(db, trade_id: int) -> Optional[Dict[str, Any]]:
    cur = db.conn.cursor()
    table = detect_trade_table(cur)
...
def main():
    ...
    print(f"[INFO] DB backend: {backend} schema={getattr(db,'pg_schema',None)}")

    trade = None
    sym = (args.symbol or "").strip()

    if args.trade_id is not None:
        trade = fetch_trade_from_db(db, args.trade_id)
        if not trade:
