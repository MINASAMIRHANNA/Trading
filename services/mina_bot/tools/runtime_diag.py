...
def main() -> int:
    ...
    # ------------------------------------------------------------------
    # DB checks (Postgres PRIMARY + SQLite legacy)
    # ------------------------------------------------------------------
    is_pg_backend = False
    try:
        backend = (os.getenv('MINA_DB_BACKEND') or os.getenv('TRADING_DB_BACKEND') or '').strip().lower()
        is_pg_backend = backend.startswith('post')
    except Exception:
        pass
    if db is not None and getattr(db, 'is_postgres', False):
        is_pg_backend = True

    if is_pg_backend and db is not None:
        try:
            schema = getattr(db, 'pg_schema', None) or (os.getenv('MINA_PG_SCHEMA') or os.getenv('TRADING_PG_SCHEMA') or '').strip() or None
            if not schema:
                # fallback based on role (matches docker defaults)
                role = (os.getenv('BOT_ROLE') or os.getenv('SERVICE_ROLE') or 'unknown').strip().lower()
                schema = f"mina_{role}"
            ...
            for tbl in must:
                try:
                    ...
                    cnt = cur.fetchone()[0]
                    print(f"[DIAG] rows {tbl}: {cnt}")
                    ok_tables.append(tbl)
                except Exception:
                    print(f"[DIAG] rows {tbl}: <missing>")
                    missing_tables.append(tbl)

            if missing_tables:
                print('[DIAG] ⚠️ missing tables:', missing_tables)

            # Extra sanity
            try:
                cur.execute(f"SELECT COUNT(*) FROM {schema}.trades WHERE status='OPEN'")
            ...
            print('[DIAG] ❌ Postgres DB check failed:', e)
            return 4
