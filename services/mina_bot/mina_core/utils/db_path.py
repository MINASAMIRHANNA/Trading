...
def describe_db_state(db_name: str = DEFAULT_DB_NAME) -> dict:
    ...
    Postgres PRIMARY: no pin files are used; we still show the deterministic DB_FILE path.
    """
    try:
        ...
        if backend.startswith('post'):
            return {
                ...
                'found': [],
                'duplicates': [],
                'note': 'postgres_primary:no_pin_files',
            }
