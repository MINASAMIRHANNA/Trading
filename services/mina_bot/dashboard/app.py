...
async def lifespan(app: FastAPI):
    ...
    from dashboard.services import bootstrap_service as _bootstrap
    try:
...
def _auth_enabled() -> bool:
    raw = os.getenv("DASHBOARD_AUTH_ENABLED", "true")
...
async def login_action(username: str = Form(...), password: str = Form(...)):
    # Zero-ENV first: credentials are stored in DB settings (WEB_USERNAME / WEB_PASSWORD).
    # Optional env fallback for docker-compose convenience.
    db_user = db.get_setting("WEB_USERNAME")
    db_pass = db.get_setting("WEB_PASSWORD")
    env_user = os.getenv("DASHBOARD_USER")
    env_pass = os.getenv("DASHBOARD_PASS")

    valid_user = str(db_user).strip() if db_user not in (None, "") else str(env_user or "admin")
    valid_pass = str(db_pass).strip() if db_pass not in (None, "") else str(env_pass or "admin")

    if username == valid_user and password == valid_pass:
        resp = JSONResponse(content={"msg": "Success"})
        resp.set_cookie(key=SESSION_COOKIE_NAME, value=SESSION_COOKIE_VALUE, httponly=True)
        return resp
    ...
    try:
        if hasattr(db, "get_recent_perf_metrics"):
            ...
            if getattr(db, "conn", None) is not None:
                ...
                if lock is None:
                    rows = [_row_to_dict(r) for r in (db.conn.execute(sql, (cutoff_ms, lim)).fetchall() or [])]  # type: ignore
                else:
                    with lock:
                        rows = [_row_to_dict(r) for r in (db.conn.execute(sql, (cutoff_ms, lim)).fetchall() or [])]  # type: ignore
    except Exception:
...
def _commands_cols() -> set:
    # Prefer shared repo (keeps logic identical across dashboards)
    try:
        from mina_core.db.repos.commands_repo import CommandsRepo
        conn = getattr(db, 'conn', None)
        if conn is None:
            return set()
        return set(CommandsRepo.cols(conn, getattr(db, 'lock', None)))
    except Exception:
        try:
            if getattr(db, 'conn', None) is None:
                return set()
...
def _insert_command_row(cmd: str, params: dict | None = None, source: str = "dashboard", trace_id: str | None = None) -> int:
    """Insert a command row and return its id (best-effort across schema versions)."""
    if getattr(db, 'conn', None) is None:
        raise RuntimeError('db.conn not available')

    # Prefer shared repo; fallback keeps legacy logic.
    try:
        from mina_core.db.repos.commands_repo import CommandsRepo
        lock = getattr(db, 'lock', None)
        return int(CommandsRepo.insert(db.conn, lock, str(cmd), params=params or {}, source=source, trace_id=trace_id))  # type: ignore
    except Exception:
        pass
    ...
    if trace_id and ('trace_id' in cols or 'traceId' in cols):
        # Some DBs may already have the column; best-effort only.
...
async def api_commands(status: str = '', cmd: str = '', limit: int = 50, since_ms: int = 0):
    ...
    # Prefer shared repo; fallback keeps legacy logic (moved to dashboard/legacy_commands.py).
    try:
        from mina_core.db.repos.commands_repo import CommandsRepo
        if getattr(db, 'conn', None) is None:
            return {'ok': False, 'items': [], 'error': 'db.conn not available'}
        lock = getattr(db, 'lock', None)
        items = CommandsRepo.list(db.conn, lock, status=st, cmd=cmdq, limit=lim, since_ms=since)  # type: ignore
        return {'ok': True, 'items': items, 'count': len(items)}
    except Exception:
        from dashboard.legacy_commands import list_commands_legacy
        return list_commands_legacy(db, status=st, cmd=cmdq, limit=lim, since_ms=since)
...
async def api_commands_queue(request: Request):
    ...
    # Trace linking (Gateway forwards X-Trace-Id; body.trace_id allowed for backward compatibility)
    trace_id = None
    try:
    ...
    if trace_id is None:
        try:
            t = (body.get('trace_id') or body.get('traceId') or '').strip()
            trace_id = t or None
        except Exception:
            trace_id = None

    try:
        cid = _insert_command_row(cmd, params=params, source='dashboard', trace_id=trace_id)
...
async def api_commands_cancel(cmd_id: int):
    ...
    try:
        ...
        try:
            from mina_core.db.repos.commands_repo import CommandsRepo
            updated, msg = CommandsRepo.cancel(db.conn, lock, cid)  # type: ignore
            return {'ok': True, 'message': msg, 'updated': int(updated or 0)}
        except Exception:
            pass

        from dashboard.legacy_commands import cancel_command_legacy
        updated, msg = cancel_command_legacy(db, cid)
        return {'ok': True, 'message': msg, 'updated': int(updated or 0)}
    except Exception as e:
        return {'ok': False, 'message': str(e), 'updated': 0}
...
async def api_commands_retry(cmd_id: int):
    ...
    try:
        ...
        # Best-effort: read original cmd for message consistency
        cmd_name = None
        try:
            if lock is None:
                r0 = db.conn.execute('SELECT cmd FROM commands WHERE id=?', (cid,)).fetchone()  # type: ignore
            else:
                with lock:
                    r0 = db.conn.execute('SELECT cmd FROM commands WHERE id=?', (cid,)).fetchone()  # type: ignore
            if r0:
                try:
                    cmd_name = str(r0[0]).strip().upper()
                except Exception:
                    cmd_name = str(getattr(r0, 'cmd', '')).strip().upper() if hasattr(r0, 'cmd') else None
        except Exception:
            cmd_name = None

        # Prefer shared repo to requeue; fallback keeps legacy logic.
        try:
            from mina_core.db.repos.commands_repo import CommandsRepo
            new_id = int(CommandsRepo.retry(db.conn, lock, cid, source='dashboard-retry'))  # type: ignore
            return {'ok': True, 'message': f'retried {cmd_name or cid}', 'new_command_id': int(new_id)}
        except Exception:
            from dashboard.legacy_commands import retry_command_legacy
            new_id, cmd = retry_command_legacy(db, cid, source='dashboard-retry')
...
async def update_settings(request: Request):
    ...
    # Prefer SettingsRepo (backend-agnostic); fallback keeps legacy DatabaseManager behaviour.
    _conn = getattr(db, "conn", None)
    _lock = getattr(db, "lock", None)
    def _set_setting(k, val):
        if _conn is not None:
            try:
                from mina_core.db.repos.settings_repo import SettingsRepo
                SettingsRepo.set(_conn, k, val, source="dashboard", bump_version=True, audit=True, lock=_lock)
                return
            except Exception:
                pass
...
async def get_settings_all():
    ...
    if conn is not None:
        try:
            from mina_core.db.repos.settings_repo import SettingsRepo
            return SettingsRepo.list_all(conn, lock=getattr(db, "lock", None))
        except Exception:
            pass
...
async def get_advanced_stats():
    from dashboard.services.reports_service import get_advanced_stats as _svc_get_advanced_stats
    return _svc_get_advanced_stats(db)
@app.get("/api/reports/daily")
async def api_reports_daily(days: int = 7):
    from dashboard.services.reports_service import daily as _svc_reports_daily
    return _svc_reports_daily(db, days=days)
@app.get("/api/reports/pnl")
async def api_reports_pnl(days: int = 30):
    from dashboard.services.reports_service import pnl as _svc_reports_pnl
    return _svc_reports_pnl(db, days=days)
@app.get("/api/reports/top_symbols")
async def api_reports_top_symbols(limit: int = 10):
    from dashboard.services.reports_service import top_symbols as _svc_reports_top_symbols
    return _svc_reports_top_symbols(db, limit=limit)
@app.get("/api/reports/winrate")
async def api_reports_winrate(days: int = 30):
    from dashboard.services.reports_service import winrate as _svc_reports_winrate
    return _svc_reports_winrate(db, days=days)
...
async def get_system_health():
    """System health payload (refactored to health_service, behavior-preserving)."""
    from dashboard.services.health_service import build_system_health_payload
    return build_system_health_payload(db=db, project_root=PROJECT_ROOT, execmon_age_seconds_fn=_execmon_age_seconds)



@app.get("/api/db_ready")
async def api_db_ready():
    """DB readiness probe (idempotent bootstrap + minimal meta)."""
    try:
        from mina_core.db.ddl import bootstrap_dashboard, list_schema_meta  # type: ignore
        bootstrap_dashboard(db.conn, lock=getattr(db, "lock", None))
        meta = {}
        try:
            meta = list_schema_meta(db.conn, lock=getattr(db, "lock", None))
        except Exception:
            meta = {}
        return {
            "status": "ok",
...
async def get_positions():
    from dashboard.services.positions_service import get_positions as _svc_get_positions
    return _svc_get_positions(db, get_real_positions, _positions_status_map, _normalize_side, _public_price)
...
# ✅ SIGNAL INBOX (approval workflow)
# =========================================
# Batch-18: slim pass — move workflow to dashboard/signals_workflow.py
...
async def api_signal_one(signal_id: int):
    r = _get_signal_by_id(int(signal_id))
    if not r:
...
async def api_signal_approve(request: Request):
    ...
    return _signals.approve_signal(db, data)
...
async def api_signal_reject(request: Request):
    ...
    if not isinstance(data, dict):
        raise HTTPException(status_code=400, detail="Invalid payload")
    return _signals.reject_signal(db, data)
...
async def api_analytics():
    from dashboard.services.analytics_service import get_analytics as _svc_get_analytics
    return _svc_get_analytics(db)
@app.get("/api/audit")
